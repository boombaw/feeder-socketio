...
const sdosen = io.of("/sync_dosen_satuan");
const eventName = "sync-dosen-satuan";
...
sdosen.on("connection", async (socket) => {
	...
	socket.on(eventName, async (params) => {
		let { kd_prodi, sms, tahun, nid } = JSON.parse(params);
		...
		try {
			...
			let jadwalDosen = await ajarDosen.getJadwalDosen(kd_jadwal);
				...
				if (error_code == 0) {
					...
					try {
						...
						if (error_code === 0 && data.length > 0) {
							...
							let id_registrasi_dosen = await idRegistrasiDosen(
								tokenFeeder,
								{
									id_dosen,
									tahun,
							...
							if (id_registrasi_dosen === null) {
								error_desc = "Penugasan Dosen tidak ditemukan";
								response.list.status = `<span class="badge rounded-pill bg-danger " style="font-size:0.8rem !important">Gagal</span>`;
							} else {
